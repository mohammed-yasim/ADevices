...
const long SYNC_TIME = 8000; // Adjust this value to change the sync time (in milliseconds)
long refresh_time;
long sync_time;
...
void loop() {
  ...
  if (millis() > sync_time) {
    ...
    if (dataChanged) {
      ...
      // Log data to Firebase as an object with a timestamp

      //UpdateMappedData(mapped_x, mapped_y, mapped_z);
      //UpdateFiltredData(filtered_x, filtered_y, filtered_z);
...
void UpdateMappedData(float x, float y, float z) {
  FirebaseJson json;
  timestamp = getTime();
  json.set("x", x);
  json.set("y", y);
  ...
  Firebase.pushJSON(update_mapped_data, "/D1/sensor_mapped/", json);
}
void UpdateFiltredData(float x, float y, float z) {
  FirebaseJson json;
  timestamp = getTime();
  json.set("x", x);
  json.set("y", y);
  json.set("z", z );
  json.set("time", getTime());
  Firebase.pushJSON(update_filtered_data, "/D1/filter_kalman/", json);
}
